...
static void bench_evio_churn(bool use_uring)
{
    ...
    int fds[NUM_WATCHERS][2];

    for (size_t i = 0; i < NUM_WATCHERS; ++i) {
        fds[i][0] = fds[i][1] = -1;
        pipe(fds[i]);
        evio_poll_init(&watchers[i], dummy_evio_cb, fds[i][1], EVIO_WRITE);
    }
...
static void bench_libev_churn(void)
{
    ...
    for (size_t i = 0; i < NUM_WATCHERS; ++i) {
        fds[i][0] = fds[i][1] = -1;
        pipe(fds[i]);
        ev_io_init(&watchers[i], dummy_libev_cb, fds[i][1], EV_WRITE);
    }
...
static void bench_libuv_churn(void)
{
    ...
    for (size_t i = 0; i < NUM_WATCHERS; ++i) {
        fds[i][0] = fds[i][1] = -1;
        pipe(fds[i]);
        uv_poll_init(loop, &watchers[i], fds[i][1]);
    }
